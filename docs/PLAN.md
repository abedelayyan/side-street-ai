...
> *Google Docs for agent sessions: watch, steer, and hand off a live coding agent with your
> whole team — no matter which lab's agent you run.*

### Why now

- Agents run tasks lasting hours to weeks, but every session is trapped on one person's laptop.
- Every shipping agent (Claude Code, Codex, Cursor) solved *single-user* steering the same way
...
| Zed | Humans co-edit code, agent is an assistant | We make the *agent session* the shared object, not the buffer |
...
mentoring — moments teams *already* crowd around one problem. Not greenfield coding.
...
*Goal: a contributor (or Claude) can clone, install, test, and ship a PR with zero friction.*

- [x] `docs/PLAN.md` (this document) and `docs/research/` grounding
- [x] `CLAUDE.md` working agreement
- [ ] Monorepo scaffold: pnpm workspaces, Turborepo, TypeScript strict, ESLint + Prettier, Vitest
- [ ] CI: GitHub Actions running lint/typecheck/test/build on every PR; branch protection on `main`
- [ ] OSS hygiene: Apache-2.0 `LICENSE`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, issue/PR templates
- [ ] ADR process: `docs/adr/0000-template.md` + ADR-0001 (Durable Objects spine) + ADR-0002 (ACP client, no own agent)
- [ ] `packages/core`: event types, zod schemas, hash-chain utilities — fully unit-tested
...
*Goal: the thinnest vertical slice that answers the existential question: can two engineers
co-steer one debugging session without producing incoherent agent behavior?*
...
*Goal: the session layer is trustworthy enough to hold real credentials in front of multiple viewers.*
...
*Goal: an on-call team resolves a real incident faster with shared steering than with
screen-share + copy-paste.*
...
*Goal: Side Street is a credible, adoptable open-source project with a public story.*
...
- [ ] Launch sequence: (1) soft-launch to the communities already asking for this — Claude Code issue #60082 thread, ACP community, Zed/Cursor discords; (2) Show HN with the demo; (3) launch blog post: *"Every coding agent solved single-user steering the same way. Nobody solved multiplayer. Here's how we did."*
- [ ] Open-core boundary stated publicly from day one: **the protocol, session layer, steering model, and self-hosting are permanently open (Apache-2.0); the hosted control plane, SSO/SCIM, and managed compliance features are the commercial layer.** Honesty here is the community moat.
...
- **PRs:** small and focused (< ~400 lines diff where possible); description says *what* and *why*; CI must be green; every PR maps to a plan deliverable or an amendment to this plan.
...
**Narrative:** *"Agent-agnostic multiplayer for coding agents — watch, steer, hand off, audit."*
...
4. **Positioning discipline:** never bash the labs — we *amplify* their agents. "Works with Claude Code, Codex, and Gemini" is the headline, not the footnote.
...
*Last amended: 2026-07-25. Amend via PR; every amendment updates this line.*
